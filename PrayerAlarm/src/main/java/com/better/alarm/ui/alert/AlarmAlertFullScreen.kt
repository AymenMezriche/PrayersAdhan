...
class AlarmAlertFullScreen : FragmentActivity() {
    ...
    override fun onCreate(icicle: Bundle?) {
        ...
        setTheme(R.style.AlarmAlertFullScreenLightTheme)
    ...
    private fun setTitle() {
        ...
        title = titleText
        findViewById<TextView>(R.id.alarm_alert_label).text = getAdhanMessage(titleText)
    }

    private fun getAdhanMessage(titleText: String): String {
        val adhanNames = listOf("Fajr", "Dhuhr", "Asr", "Maghrib", "Isha")
        val prayerArNames = listOf("الفجر", "الظهر", "العصر", "المغرب", "العشاء")
        val message = "حان الان وقت صلاة"
        for (name in adhanNames) {
            if (titleText.contains(name, ignoreCase = true)) {
                val index = adhanNames.indexOf(name)
                return "$message ${prayerArNames[index]}"
            }
        }
        return titleText
    }
