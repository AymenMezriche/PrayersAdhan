...
class KlaxonPlugin(
    ...
    override fun go(
        ...
        val volumeSub =
            volume.subscribe { currentVolume -> player?.setPerceivedVolume(currentVolume) }

        disposable =
            CompositeDisposable(
                callSub,
                volumeSub,
                Disposables.fromAction { player?.stopAndCleanup() })
        return disposable
    }
...
//                    setDataSourceFromResource(R.raw.fallbackring)
    ...
    private fun playInCallAlarm() {
        ...
        player?.run {
            ...
            // Using adhan1 as in-call alarm as it is less jarring than the original in_call_alarm sound
            setDataSourceFromResource(R.raw.adhan1)
            startAlarm()
        }
    }
