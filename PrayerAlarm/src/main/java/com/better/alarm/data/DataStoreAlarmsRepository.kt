...
class DataStoreAlarmsRepository(
                ...
                prev.alarms
                    ...
                    .forEach { (id, prevValue) ->
                      logger.debug { "changed $prevValue => ${newData[id]}" }
                        Log.d("DataStoreAlarmsRepo", "changed $prevValue => ${newData[id]}" )
                    }
                // added
                newData
  ...
  suspend fun updateTimeSilently(id: Int, newHour: Int, newMinute: Int) {
    withContext(ioScope.coroutineContext) {
      alarmsByIdState.update { alarms ->
        val alarm = alarms[id]
        if (alarm != null) {
          alarms.plus(id to alarm.copy(hour = newHour, minutes = newMinute))
        } else {
          alarms
        }
      }
      logger.debug { "updateTimeSilently(): updated alarm $id → $newHour:$newMinute" }
    }
  }
