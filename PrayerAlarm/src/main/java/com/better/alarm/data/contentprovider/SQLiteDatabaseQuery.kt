    ...
    contentResolver
        ...
        ?.use { cursor ->
          if (cursor.moveToFirst()) {
            do {
                val alarm = fromCursor(cursor)
                Log.d("SQLiteDatabaseQuery", "Alarm ID: ${alarm.id}, Time: ${alarm.hour}:${alarm.minutes}, Enabled: ${alarm.isEnabled}, Days: ${alarm.daysOfWeek} , name = ${alarm.label}")
